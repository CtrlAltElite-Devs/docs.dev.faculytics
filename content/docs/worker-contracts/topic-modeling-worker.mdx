...
- **Request:** Sent as `{ "input": ...request body... }` to the RunPod `/runsync` endpoint.
- **Response:** RunPod returns `{ "output": ...response body... }` on completion. Topic modeling routinely exceeds the `/runsync` timeout (~30s), so RunPod returns `{"id":"...","status":"IN_QUEUE"}` and the processor polls `/status/{id}` every 5 seconds until done (up to `BULLMQ_TOPIC_MODEL_HTTP_TIMEOUT_MS`, default 300s).
