...
Institutional sync (categories, courses, enrollments) uses a BullMQ-based composite job instead of individual cron jobs. See [Institutional Sync Workflow](/docs/workflows/institutional-sync) for the full flow.
...
See [Analytics Module](/docs/architecture/analytics) for full architecture.
...
See [Report Generation Workflow](/docs/workflows/report-generation) for the full async flow.
